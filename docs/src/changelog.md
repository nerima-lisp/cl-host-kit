...
- Atomic text and octet replacement plus binary `copy-file` and `copy-path`
  with an explicit no-replace default, opt-in replacement, and optional
  `fsync` durability, alongside scoped advisory file locking with bounded
  waits.
