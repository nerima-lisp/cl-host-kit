...
| `uiop:split-string` | `split-string` | Identical for the org's observed usage. `:separator` accepts a character, a list of characters, or a string (each character an independent delimiter), and `:max` is supported with the same capped-segmentation behavior as uiop. |
